...
const Ciudades = () => {
    ...
    const confirmDelete = (ciudadId, token) => {
        Swal.fire({
			...
			if (result.isConfirmed) {
				actions.eliminarCiudad(ciudadId, token)
			Swal.fire({
				title: "Deleted!",
    ...
    const handleDelete = async (ciudadId) => {
        let user = localStorage.getItem("nombre")
        let newToken = await actions.renovarToken(user)
        confirmDelete(ciudadId, newToken.token)
    }

    useEffect(() => {
        actions.getCiudadesyPaises()
    }, [])
        ...
        return (
            <Container fluid className="bg-black pt-4" style={{ "height": "100vh" }}>
                ...
                <Table striped bordered hover className="w-50 mx-auto">
                    ...
                    <tbody>
                        {store.ciudadesyPaises.map((ciudad, id) => (
                            <tr key={id}>
                                <td className="text-light text-center">{ciudad.idCiudad}</td>
                                <td className="text-light text-center">{ciudad.nombre_de_pais}</td>
                                <td className="text-light text-center">{ciudad.nombre_de_ciudad}</td>
                                <td>
                                    <Row>
                                        <Col className="d-flex justify-content-center align-items-center">
                                            <Button onClick={() => handleDelete(ciudad.idCiudad)} variant="outline-danger">
                                        ...
                                        <Col className="d-flex justify-content-center align-items-center">
                                            <Button variant="outline-primary" href={/crearCiudad/ + ciudad.idCiudad}>
